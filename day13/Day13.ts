...
function parseInput(inputLines : string[]) : ParsedInput {
    ...
    if (accLines.length > 0) splitStrings.push(accLines);

    return {grids : splitStrings};
}

function findVerticalMirror(grid : string[]) : number{
	...
	if (checkSymmetryFromX(i)) return i+1;
    }    

    return -1;

    function checkSymmetryFromX(centerX : number) : boolean {
	let i = centerX;
	let j = centerX+1;
	...
	while ((i >= 0) && (j < grid[0].length)) {
	    ...
	    if (leftColumn.toString() !== rightColumn.toString()) return false;
	    i -= 1;
	    j += 1;
	}
...
function findHorizontalMirror(grid : string[]) : number {
    //Returns the index of the line below the mirror
    
    const gridHeight = grid.length;

    for (let i = 0; i < gridHeight-1; i++) {
	if (checkSymmetryFromY(i)) return i+1;
    }    

    return -1;
    
    function checkSymmetryFromY(centerY : number) : boolean {
	let i = centerY;
	let j = centerY+1;
	...
	while ((i >= 0) && (j < grid.length)) {
	    ...
	    if (topRow.toString() !== bottomRow.toString()) return false;
	    i -= 1;
	    j += 1;
	}
