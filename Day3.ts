...
function part1Main() {
    ...
    for (let i = 0; i < inputRows; i++) {
        ...
        for (const substringIndex of indexes) {
            ...
            if (symbolTest) resultSum += Number.parseInt(lines[i].slice(substringIndex[0], substringIndex[1]));
        }
    }
...
function part2Main() {
    ...
    for (let i = 0; i < inputRows; i++) {
        ...
        for (let j = i-1; j <= i+1; j++) {
            if (j === -1) windowLines.push("");
            else if (j === inputRows) windowLines.push("");
            else windowLines.push(lines[j]);
        }
...
function testForSymbols(inputLines: string[], indices: [number, number]) {
    const trimmedLines = inputLines.map((line: string) => {
    ...
    for (const line of trimmedLines) {
        if (symbolRegexp.test(line)) return true;
    }

    return false;
}

function getGearRatiosForLine(inputLines: string[]) {
    ...
    for (const match of matches) {
        ...
        if (adjacentNumbers.length === 2) ratioSum += adjacentNumbers[0] * adjacentNumbers[1];
    }

    return ratioSum;
}

function getAdjacentNumbers(inputLines: string[], indexOfCenter: number) {
    ...
    for (const match of matches) {
        ...
        if (matchBeginningIndex === undefined) process.exit(2);
...
function getMatchesFromIterators(iterators : IterableIterator<RegExpMatchArray>[]) {
    const matches = [];
...
function convertToRange(rangeMin: number, rangeMax: number) {
    const range = [];
    for (let i = rangeMin; i <= rangeMax; i++) range.push(i);
    return range;
}

function findNumberIndexes(inputLine: string) {
