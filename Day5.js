...
function parseMapLines(fullInput, startingLineIndex, headerRegex) {
    ...
    while (mapMatch !== null) {
        ...
        lineIndex++;
        if (lineIndex >= fullInput.length)
            break;
        mapMatch = fullInput[lineIndex].match(MAPEXP);
    }
    return maps;
}
function routeSeedToLocation(seedNumber, mapSet) {
...
function sourceToDestination(startingNumber, mapArray) {
    let currentResult = startingNumber;
    for (const map of mapArray) {
        if (map.sourceStart > currentResult)
            continue;
        if (map.sourceStart + map.range <= currentResult)
            continue;
...
function destinationToStart(startingNumber, mapArray) {
    let currentResult = startingNumber;
    for (const map of mapArray) {
        if (map.destinationStart > currentResult)
            continue;
        if (map.destinationStart + map.range <= currentResult)
            continue;
        //console.log('Found route. Current: %d, SourceStart: %d, DestinationStart: %d, Range: %d', 
        //currentResult, map.sourceStart, map.destinationStart, map.range);
...
function getBreakpoints(mapSet, seedRanges) {
    const orderedMapSet = [
        mapSet.humidityToLocationMaps,
        mapSet.temperatureToHumidityMaps,
        mapSet.lightToTemperatureMaps,
    ...
    for (const currentSet of orderedMapSet) {
        ...
        //add new breakpoints
        if (!breakpoints.includes(0))
            breakpoints.push(0);
        for (const map of currentSet)
            breakpoints.push(map.sourceStart);
    }
    //Remove any numbers that don't fit in seed ranges
    breakpoints = breakpoints.filter((seedNumber) => {
        for (const seedRange of seedRanges) {
            if (seedRange.start > seedNumber)
                continue;
            if (seedRange.start + seedRange.range <= seedNumber)
                continue;
            return true;
        }
        return false;
    ...
    for (const seedRange of seedRanges) {
        if (!breakpoints.includes(seedRange.start))
            breakpoints.push(seedRange.start);
    }
    return breakpoints;
}
function sortMapsBySource(mapArray) {
    const sortedArray = [...mapArray].sort((a, b) => a.sourceStart - b.sourceStart);
    return sortedArray;
}
function sortMapsByDestination(mapArray) {
